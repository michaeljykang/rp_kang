async function getData(){
    const response = await fetch('data/accuracy-data.csv');    // .. to moveup one level in folder structure
    const data = await response.text()                          // CSV to Text format
    console.log(data);

    const xGroups = [];     // x-axis label = groups
    const yExpAccuracy = [];   // y-axis experimental gruop time values
    const yCtrlAccuracy = [];  // y-axis control group time values
    ...
    table.forEach(row => {
        ...
        const accuracy = parseFloat(columns[1]);        // Convert time to float
        if(group.includes('control')){
            yCtrlAccuracy.push(accuracy);                  // Push time values to array
        } else {
            yExpAccuracy.push(accuracy);                     
        }
        console.log(group, accuracy);
    });

    return {xGroups, yCtrlAccuracy, yExpAccuracy};    // Use {} to return multiple values as a single object
}
...
async function createChart(){
    ...
    const myChart = new Chart(accuracyChart, {
        type: 'bar',
        data: {
        labels: data.xGroups,
        datasets: [{
            label: 'Experimental Group Accuracy',
            data: data.yExpAccuracy,
            backgroundColor:  'rgba(255, 0, 102, 0.2)',    // Color for data marker
            borderColor:      'rgba(255, 0, 102, 1)',      // Color for data marker border 
            borderWidth: 1
        }, {
            label: 'Control Group Accuracy',
            data: data.yCtrlAccuracy,
        ...
        options: {
            ...
            plugins: {                  // Display options for title and legend
                title: {
                    display: true,
                    text: 'Planaria Accuracy in Finishing the Maze',
                    font: {
                        size: 24,
                    },
                    color: '#black',
